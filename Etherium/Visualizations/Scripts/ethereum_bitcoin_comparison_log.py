...
def create_comparison_plot():
    """Create the comparison plot with two subplots in log scale"""
    ...
    # Plot 1: Ethereum Full Price History (Log Scale)
    ax1.plot(eth_data['Date'], eth_data['Price'], color='#627EEA', linewidth=1.5, alpha=0.8)
    ax1.set_title('Ethereum (ETH) Full Price History - Log Scale', fontsize=16, fontweight='bold', pad=20)
    ax1.set_ylabel('Price (USD) - Log Scale', fontsize=12)
    ...
    # Plot 2: Bitcoin Price from Start to 2019 (Log Scale)
    ax2.plot(btc_data['Date'], btc_data['Price'], color='#F7931A', linewidth=1.5, alpha=0.8)
    ax2.set_title('Bitcoin (BTC) Price History: 2010-2020 - Log Scale', fontsize=16, fontweight='bold', pad=20)
    ax2.set_ylabel('Price (USD) - Log Scale', fontsize=12)
    ...
    filename = 'Etherium/Visualizations/Images/ethereum_bitcoin_comparison_log.png'
    ...
    print("Previous log scale image has been overwritten.")
...
def main():
    ...
    print("Creating log scale comparison plot...")
    
    fig = create_comparison_plot()
    
    if fig:
        print("Log scale comparison plot created successfully!")
    else:
        print("Failed to create log scale comparison plot.")
