...
def load_and_prepare_data():
    ...
    # Load the data - use the most recent combined file
    data_dir = "Metals/Silver/Data"
    
    # Find the most recent combined file
    files = [f for f in os.listdir(data_dir) if f.startswith("Silver_Price_Complete_")]
    if not files:
        print("Error: No combined silver price data file found.")
        return None
...
def fit_models(df):
    ...
    # Linear model
    try:
        popt_linear, _ = curve_fit(linear_model, x, y, p0=[0.01, 5])
    ...
    # Exponential model
    try:
        popt_exp, _ = curve_fit(exponential_model, x, y, p0=[5, 0.001, 5])
    ...
    try:
        popt_poly, _ = curve_fit(polynomial_model, x, y, p0=[1e-8, 1e-5, 0.01, 5])
    ...
    # Logarithmic model
    try:
        popt_log, _ = curve_fit(logarithmic_model, x, y, p0=[10, 0.01, 5])
    ...
    # Power model
    try:
        popt_power, _ = curve_fit(power_model, x, y, p0=[1, 0.5, 5])
...
def create_visualization(df, models):
    ...
    ax1.set_title('Silver Price Growth Models Comparison', fontsize=16, fontweight='bold')
    ax1.set_xlabel('Date')
    ...
    plot_file = f"Metals/Silver/Models/Growth/Results/silver_growth_models_analysis_{timestamp}.png"
...
def save_results(models, df):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = f"Metals/Silver/Models/Growth/Results/silver_growth_models_results_{timestamp}.txt"
    
    with open(results_file, 'w') as f:
        f.write("SILVER PRICE GROWTH MODEL ANALYSIS\n")
        f.write("=" * 50 + "\n\n")
...
def main():
    ...
    print("Silver Price Growth Model Analysis")
    print("=" * 40)
