...
def dynamic_gbm_fair_value_start(T, num_paths=1000, num_steps=3653):
    """Run Dynamic GBM simulation starting at formula fair value"""
    print(f"\nRunning Fair Value Start Dynamic GBM Simulation")
    print(f"="*60)
    
    # Calculate starting price from formula (fair value)
    ...
    # Create output file
    paths_filename = f'Results/Bitcoin/bitcoin_gbm_fair_value_start_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
def analyze_fair_value_start_results(paths_filename):
    """Analyze the fair value start GBM results"""
    print(f"\nAnalyzing fair value start GBM results...")
...
def save_fair_value_start_summary(results, paths_filename):
    """Save summary statistics for fair value start results"""
    summary_filename = paths_filename.replace('.csv', '_summary.csv')
...
def main():
    """Main function"""
    print("Bitcoin Fair Value Start Dynamic GBM Simulation")
    print("="*60)
    
    # Run fair value start GBM simulation
    paths_file = dynamic_gbm_fair_value_start(T=10, num_paths=1000, num_steps=3653)
    
    # Analyze results
    results = analyze_fair_value_start_results(paths_file)
    
    # Save summary statistics
    summary_file = save_fair_value_start_summary(results, paths_file)
    
    # Get formula fair value for comparison
    fair_value, _, _ = calculate_formula_fair_value()
    
    print(f"\n" + "="*50)
    print("FAIR VALUE START GBM SIMULATION SUMMARY")
    print("="*50)
    ...
    print("\n✅ Fair value start GBM simulation completed successfully!")
    print("Note: Simulation starts at formula fair value instead of actual market price")
