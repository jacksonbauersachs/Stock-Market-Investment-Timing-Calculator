...
# Load the complete historical Bitcoin data
...
print('='*60)
print('BITCOIN RAINBOW CHART (AUTOMATIC FLATTENING BANDS)')
print('='*60)
...
with open('Models/Growth Models/bitcoin_growth_model_coefficients_day365.txt', 'w') as f:
    f.write(f'a = {slope}\n')
...
df['log_dev'] = log_dev  # Add this line to fix the error
...
with open('Models/Growth Models/bitcoin_rainbow_band_formulas.txt', 'w') as f:
    f.write(f'# Bitcoin Rainbow Band Formulas\n')
    f.write(f'# Last updated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n')
    f.write(f'# Data range: {df["Date"].min().strftime("%Y-%m-%d")} to {df["Date"].max().strftime("%Y-%m-%d")}\n')
    f.write(f'# R² = {R2:.6f}\n\n')
...
ax.set_title('Bitcoin Rainbow Chart (Auto-Flattening Bands, Start Day 365)', fontsize=18, fontweight='bold')
...
plt.savefig('Results/Bitcoin/bitcoin_rainbow_chart_day365_autoflat.png', dpi=300, bbox_inches='tight')
plt.show()

print(f"\nRainbow chart saved to: Results/Bitcoin/bitcoin_rainbow_chart_day365_autoflat.png")
print(f"Model coefficients saved to: Models/Growth Models/bitcoin_growth_model_coefficients_day365.txt")
print(f"Band formulas saved to: Models/Growth Models/bitcoin_rainbow_band_formulas.txt")
print(f"\nModel Summary:")
