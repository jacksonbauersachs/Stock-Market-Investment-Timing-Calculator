...
def get_recent_bitcoin_data():
    """Get recent Bitcoin data to understand current market conditions"""
    print("Fetching recent Bitcoin data...")
    ...
    current_volatility = btc['Close'].pct_change().std() * np.sqrt(252) * 100  # Annualized volatility
    
    # Calculate recent growth rate (last 6 months)
    six_months_ago = btc['Close'].iloc[-126] if len(btc) > 126 else btc['Close'].iloc[0]
    recent_growth_rate = ((current_price / six_months_ago) ** (252/126) - 1) * 100
    
    print(f"Current Bitcoin price: ${current_price:,.2f}")
    print(f"Current annualized volatility: {current_volatility:.1f}%")
    print(f"Recent annualized growth rate: {recent_growth_rate:.1f}%")
    
    return btc, current_price, current_volatility, recent_growth_rate
...
def analyze_current_market_conditions():
    ...
    btc, current_price, volatility, growth_rate = get_recent_bitcoin_data()
    
    # Market condition indicators
    print(f"\nMarket Conditions:")
    print(f"- Price: ${current_price:,.2f}")
    print(f"- Volatility: {volatility:.1f}% (much lower than historical 80-120%)")
    print(f"- Recent Growth: {growth_rate:.1f}% annualized")
    
    # Volatility comparison
    print(f"\nVolatility Context:")
    print(f"- Current: {volatility:.1f}%")
    ...
    return btc, current_price, volatility, growth_rate, overvaluation
...
def simulate_lump_sum_strategies(current_price, scenarios, lump_sum_amount=10000):
    """Simulate different lump sum strategies across scenarios"""
    print("\n" + "="*60)
    print("LUMP SUM STRATEGY ANALYSIS")
    print("="*60)
    
    strategies = {
        'Invest All Immediately': {'immediate': 1.0, 'dca_months': 0},
        'DCA over 3 months': {'immediate': 0.0, 'dca_months': 3},
        'DCA over 6 months': {'immediate': 0.0, 'dca_months': 6},
        'DCA over 12 months': {'immediate': 0.0, 'dca_months': 12},
        '70% Immediate + 30% DCA (6mo)': {'immediate': 0.7, 'dca_months': 6},
        '50% Immediate + 50% DCA (6mo)': {'immediate': 0.5, 'dca_months': 6},
        'Wait for 10% drop': {'immediate': 0.0, 'dca_months': 0, 'wait_drop': 0.1},
        'Wait for 20% drop': {'immediate': 0.0, 'dca_months': 0, 'wait_drop': 0.2}
    }
    ...
    for scenario_name, scenario_params in scenarios.items():
        ...
        for strategy_name, strategy_params in strategies.items():
            ...
            for path in range(n_paths):
                ...
                total_coins = 0
                remaining_cash = lump_sum_amount
                ...
                if strategy_params['immediate'] > 0:
                    ...
                    remaining_cash -= immediate_amount
                
                # DCA portion
                if strategy_params['dca_months'] > 0:
                    dca_amount = remaining_cash / strategy_params['dca_months']
                    for month in range(strategy_params['dca_months']):
                        if month * 30 < len(path_prices):
                            coins_bought = dca_amount / path_prices[month * 30]
                            total_coins += coins_bought
                
                # Wait for drop strategy
                if 'wait_drop' in strategy_params:
                    ...
                    for day, price in enumerate(path_prices):
                        if price <= target_price and not invested:
                            coins_bought = lump_sum_amount / price
                            total_coins += coins_bought
                            invested = True
                            break
                    
                    if not invested:  # Never hit target, invest at end
                        coins_bought = lump_sum_amount / path_prices[-1]
                        total_coins += coins_bought
...
def analyze_results(results, scenarios):
    ...
    print("\n" + "="*80)
    print("COMPREHENSIVE LUMP SUM STRATEGY RESULTS")
    print("="*80)
...
def main():
    ...
    print("Current Market Conditions Assessment")
    print("="*60)
    
    # Get current market data
    btc, current_price, volatility, growth_rate, overvaluation = analyze_current_market_conditions()
    
    # Create realistic scenarios
    scenarios = create_realistic_scenarios(current_price, volatility, growth_rate)
    
    # Simulate strategies
    results = simulate_lump_sum_strategies(current_price, scenarios)
    ...
    output_file = f'Results/Bitcoin/lump_sum_analysis_current_market_{timestamp}.txt'
    
    with open(output_file, 'w') as f:
        f.write("BITCOIN LUMP SUM INVESTMENT ANALYSIS\n")
        f.write("Current Market Conditions Assessment\n")
        f.write("="*60 + "\n")
